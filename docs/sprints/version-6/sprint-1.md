# Version 6 Sprint 1: Stripe Brand Payments and Wallet Top-Ups
...
Sprint goal: Integrate Stripe Payments for brand cards and wallet top-ups.

## Prerequisites Already Achieved

- Version 5 has completed and its sprint/module docs have been reviewed.
...
- Integrate Stripe Payments for brand cards and wallet top-ups.
- Preserve the existing manual ledger as the source of product-visible truth.
...
- Wallet ledger entries must be append-only with balance derivation, external payment references, and reconciliation status.
- Create payment_intents/records with provider, provider_id, amount, currency, status, workspace_id, and ledger_entry_id.
- Never trust client-side payment status without server confirmation.
- Store Stripe customer/account/payment identifiers, not raw card details.
- Webhook events must be idempotent and persisted before processing.
...
- /brand/billing/wallet.
- Payment method and top-up routes.
- Stripe checkout/payment method return routes and webhook route.
...
- requires_action, processing, succeeded, failed, cancelled, refunded.
- connected, requires_action, processing, succeeded, failed.
...
Expected module documentation files for this sprint: docs/modules/billing.md, docs/modules/wallet.md, docs/modules/payments.md, docs/modules/stripe.md.
...
- Brands can add/remove payment cards through Stripe.
- Wallet top-ups create internal ledger entries after confirmed payment.
- Failed, pending, and succeeded payment states are handled explicitly.
- Admin can reconcile Stripe payment IDs against wallet entries.
