# Version 6 Sprint 7: LinkedIn Import, Calendar Booking, Slack Community, and Integration Health
...
Sprint goal: Add remaining operational integrations from the Naano-inspired workflow.

## Prerequisites Already Achieved

- Version 6 Sprint 6 has completed and affected module docs have been updated.
...
- Add remaining operational integrations from the Naano-inspired workflow.
- Keep graceful fallbacks when an integration is disabled.
...
- Create linkedin_import_jobs, profile snapshots, audience snapshots, source timestamps, and sync status.
- Respect compliance limits and allow manual fallback.
- Create calendar_events with provider, provider_event_id, workspace_id, campaign_id, attendees, start/end, status.
- Create community_posts/resources only if managed inside the app. Store external community URL/config when integration arrives later.
...
- Creator LinkedIn settings and admin import review.
- Booking setup, strategy call detail, and managed ops call views.
- /creator/community and public community landing page if needed.
...
- not_connected, importing, complete, failed, stale, manual_only.
- scheduled, rescheduled, cancelled, completed, sync_failed.
...
Expected module documentation files for this sprint: docs/modules/linkedin-import.md, docs/modules/calendar.md, docs/modules/community.md, docs/modules/integrations.md, docs/modules/admin.md.
...
- LinkedIn import or controlled import workflow can refresh profile/audience data where compliant.
- Google Calendar booking creates or links campaign strategy calls.
- Slack community CTA can invite or route creators to community.
- Integration health pages show status, last sync, errors, and configuration state.
