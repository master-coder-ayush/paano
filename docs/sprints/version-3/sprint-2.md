# Version 3 Sprint 2: Pixel Site Keys and Installation Snippet
...
Sprint goal: Add per-space pixel site keys and an installable script snippet.

## Prerequisites Already Achieved

- Version 3 Sprint 1 has completed and affected module docs have been updated.
...
- Add per-space pixel site keys and an installable script snippet.
- Make pixel setup visible in brand settings/integrations.
...
- Create spaces with workspace_id, brand_id, name, website, description, industry, ICP, target_locations, default_cta_url, tracking_key_id, status.
- Script versions should be documented; store script release/version only if rollout control is needed.
...
- /brand/spaces, /brand/spaces/new, /brand/spaces/[spaceId].
- /n.js or equivalent static/script route.
...
Expected module documentation files for this sprint: docs/modules/pixel.md, docs/modules/spaces.md, docs/modules/settings.md, docs/modules/public-script.md.
...
- Each space has a unique pixel key.
- Brand users can copy the snippet using the Paano global function pattern.
- Brand users can regenerate keys with a clear warning that old keys stop working.
- Docs explain where to install the script and how to test it.
