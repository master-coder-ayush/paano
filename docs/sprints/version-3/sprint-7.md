# Version 3 Sprint 7: Tracking Admin Tools, Documentation, and V3 QA

## Version Context

Version 3: Tracking, Attribution, and Real Results

Version goal: Replace manual performance reporting with first-party tracking links, pixel events, attribution, and reporting.

Sprint goal: Add tracking diagnostics and complete V3 documentation.

## Prerequisites Already Achieved

- Version 3 Sprint 6 has completed and affected module docs have been updated.
...
- Developers must review earlier sprint files in docs/sprints/version-3 when this is not the first sprint in the version.

## Expected Behavior

- Add tracking diagnostics and complete V3 documentation.
- Regression-test tracking from click through conversion.
...
- Create or update a repeatable QA checklist for the whole version, including setup data, user roles, and pass/fail expectations.
- Record known limitations and deferred items explicitly so managers do not mistake them for defects.
- Document attribution precedence, duplicate handling, bot filtering, privacy handling, and debugging workflow.
...
- Create tracking_links and tracking_clicks with workspace, space, campaign, creator, collaboration, published_post, destination, token, clicked_at, request metadata, and bot score.
- Create pixel_site_keys, pixel_events, conversion_events, and pixel_debug_events.
- Pixel keys must be revocable and scoped to a space.
- No product database changes unless docs are generated from database-backed metadata.
- Seed data and test fixtures should be documented. QA-only data must be clearly isolated from production-like data.
...
- /r/[token] redirect route and admin tracking inspector.
- /n.js public script, /api/pixel/events, /brand/settings/integrations/pixel.
- Docs routes only if API docs are served in-app.
- QA scenarios should cover every route introduced in the current version.
...
- active, disabled, expired, invalid.
- active, revoked, test_mode, invalid_key.
- draft, reviewed, published for docs lifecycle if tracked.
- happy_path, validation_error, permission_denied, empty_state, failure_state.
...
Expected module documentation files for this sprint: docs/modules/admin.md, docs/modules/tracking.md, docs/modules/pixel.md, docs/modules/docs.md, docs/modules/qa.md.
...
- Admin can inspect tracking links, pixel events, attribution records, and recent failures.
- Testers can simulate click, signup, purchase, duplicate event, missing key, regenerated key, and disabled link scenarios.
- Module docs cover pixel contract, event schema, attribution rules, and privacy behavior.
- V3 can be demoed end to end without payment or CRM integrations.
