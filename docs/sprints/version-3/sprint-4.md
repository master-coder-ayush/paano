# Version 3 Sprint 4: Attribution Engine and Conversion Funnel
...
Sprint goal: Attribute conversion events to creator posts and campaigns using first-party rules.

## Prerequisites Already Achieved

- Version 3 Sprint 3 has completed and affected module docs have been updated.
...
- Attribute conversion events to creator posts and campaigns using first-party rules.
- Create the first reliable funnel from click to conversion.
...
- Create attribution_records with conversion_event_id, tracking_click_id, campaign_id, creator_id, post_id, attribution_model, confidence, and explanation.
- Attribution recalculation must be traceable.
- Create result_snapshots or reporting views over published_posts, tracking_clicks, pixel_events, conversion_events, and attribution_records.
- Manual metric overrides must identify source, actor, and timestamp.
- Event tables require event_id/idempotency_key, type, payload JSON, source, received_at, processed_at, and processing_status.
- Store analytics inputs in normalized event/result tables and derive rollups by workspace, campaign, creator, post, and date.
- Avoid storing personally identifiable data in aggregate tables.
...
- /brand/results attribution views and /admin/attribution inspector.
- /brand/results, /creator/analytics, campaign/post result details.
- Event ingestion and admin event viewer routes.
- Analytics dashboards and export endpoints.
...
- attributed, unattributed, duplicate, recalculated, disputed.
- no_data, partial_data, complete, stale, export_requested.
- received, processed, duplicate, failed, ignored.
- fresh, delayed, partial, failed.
...
Expected module documentation files for this sprint: docs/modules/attribution.md, docs/modules/results.md, docs/modules/events.md, docs/modules/analytics.md.
...
- Attribution connects conversions to space, campaign, creator, collaboration, and post when a valid click exists.
- Funnel displays click -> lead/signup -> purchase stages.
- Attribution handles duplicate events and missing identifiers deterministically.
- Admin can inspect attribution decisions.
