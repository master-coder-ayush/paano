# Version 3 Sprint 3: Event Ingestion, Identity Stitching, and Debug Mode
...
Sprint goal: Ingest pageview and conversion events from the pixel.

## Prerequisites Already Achieved

- Version 3 Sprint 2 has completed and affected module docs have been updated.
...
- Ingest pageview and conversion events from the pixel.
- Associate events with prior tracking clicks when possible.
...
- Store hashed or normalized identifiers with explicit privacy rules. Link anonymous visits to known identities only through documented attribution rules.
- Persist consent mode, data retention settings, and PII suppression flags where required.
- Document which fields are PII and which views may expose them.
...
- No direct user route unless identity debug tooling is introduced.
- Privacy settings and admin/privacy review pages if introduced.
...
- anonymous, identified, merged, suppressed.
- allowed, restricted, suppressed, expired.
...
Expected module documentation files for this sprint: docs/modules/pixel.md, docs/modules/events.md, docs/modules/identity.md, docs/modules/privacy.md.
...
- Pixel accepts pageview, lead, signup, trial_started, purchase, and custom conversion events.
- Events queued before script load are processed once loaded.
- Email identity is normalized and stored according to privacy policy.
- Debug mode shows recent test events without polluting production reports.
