# Version 5 Sprint 6: Managed Campaign Ops and Internal Automation
...
Sprint goal: Build the internal managed-campaign operating system without calendar or AI integrations.

## Prerequisites Already Achieved

- Version 5 Sprint 5 has completed and affected module docs have been updated.
...
- Build the internal managed-campaign operating system without calendar or AI integrations.
- Use tasks, notes, owners, and workflow states.
...
- Create managed_campaigns, ops_tasks, campaign_strategy_notes, account_manager assignments, and internal_status fields.
...
- Create tasks with workspace_id, owner_user_id, entity_type, entity_id, due_at, priority, status, and completion metadata.
- Create campaigns with workspace_id, brand_id, space_id, name, goal, budget_amount, currency, ICP, target_regions, CTA URL, status, starts_at, ends_at.
- Campaign status changes should be recorded in campaign_activity or audit_logs once workflow states become meaningful.
...
- /admin/managed-campaigns and client-facing managed campaign status page.
- /admin, /admin/review, /admin/support, and feature-specific admin detail routes.
- Task lists in admin/ops and workspace dashboards.
- /brand/campaigns, /brand/campaigns/new, /brand/campaigns/[campaignId].
...
- intake, strategy, sourcing, briefing, coordinating, live, reporting, complete, blocked.
- new, in_review, approved, rejected, needs_changes, archived for reviewable records.
- open, blocked, done, cancelled, overdue.
- draft, active, completed, cancelled, plus later briefing, sourcing, inviting, draft_review, scheduled, live.
...
Expected module documentation files for this sprint: docs/modules/managed-ops.md, docs/modules/admin.md, docs/modules/tasks.md, docs/modules/campaigns.md.
...
- Ops dashboard tracks strategy calls, campaign notes, creator sourcing, shortlists, brief approvals, creator coordination, reporting notes, and account manager assignment.
- System creates internal tasks for pending creator actions, approvals, pacing issues, budget exhaustion, and reporting due dates.
- Client-facing managed campaign status summarizes internal progress safely.
- Rules are configurable enough to avoid hard-coded one-off behavior.
