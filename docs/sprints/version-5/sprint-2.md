# Version 5 Sprint 2: API Keys and Public API Foundation
...
Sprint goal: Create first-party API key management and read/write API endpoints for future integrations.

## Prerequisites Already Achieved

- Version 5 Sprint 1 has completed and affected module docs have been updated.
...
- Create first-party API key management and read/write API endpoints for future integrations.
- No external systems are connected in this sprint.
...
- Document authentication, authorization, rate limits, pagination, idempotency, error codes, and example payloads.

## Database and Data Model Changes

- Database changes required: Yes. This sprint introduces or changes persisted product data.
- Create api_keys with hashed_key, workspace_id, scopes, created_by, last_used_at, revoked_at.
- Add api_request_logs for security-sensitive endpoints.
...
- Store security-sensitive changes in audit_logs. API keys and tokens must be hashed/encrypted as appropriate.
...
- /api/v1/* and settings pages for API keys.
- /settings/integrations and provider-specific setup pages.
- /settings/security and admin security review routes.
...
- active, revoked, locked, suspicious.
...
Expected module documentation files for this sprint: docs/modules/api.md, docs/modules/integrations.md, docs/modules/security.md, docs/modules/docs.md.
...
- Workspace admins can create, revoke, and rotate API keys.
- API exposes campaigns, creators, collaborations, results, and events according to permissions.
- Requests are authenticated, rate-limited, and audit-logged.
- API docs define request/response contracts and error codes.
