# Version 4 Sprint 5: Public Calculators
...
Sprint goal: Build local, deterministic calculators for acquisition and education.

## Prerequisites Already Achieved

- Version 4 Sprint 4 has completed and affected module docs have been updated.
...
- Build local, deterministic calculators for acquisition and education.
- Use stored/static benchmark assumptions rather than external services.
...
- Create public_tool_submissions with tool_type, input JSON, output JSON, email, status, source_page, created_at.
- For calculators, store submissions only if lead capture or history is required.
- Store calculator assumptions and benchmark values in code or benchmark tables. If persisted, version every assumption set.
- Create benchmark_sets with version, source, effective_date, metric definitions, and values if benchmark data is database-backed.
...
- /free-tools and individual calculator/search routes.
- /free-tools/linkedin-creator-worth-calculator, /linkedin-engagement-rate-calculator, /sponsored-post-delivery-odds-estimator, /creator-campaign-budget-planner.
- /reports, /benchmarks/*, calculator pages.
...
- new, calculated, researching, sent, closed.
- input, calculated, validation_error.
...
Expected module documentation files for this sprint: docs/modules/public-tools.md, docs/modules/calculators.md, docs/modules/benchmarks.md.
...
- Creator worth calculator estimates sponsored-post range from followers, reactions, comments, posting frequency, and niche.
- Engagement rate calculator computes by followers and optionally impressions.
- Delivery odds estimator uses internal/static price-band data.
- Budget planner estimates booked posts, published posts, and effective cost per published post.
