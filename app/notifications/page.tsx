...
import { demoNotifications } from "@/lib/sprint7";
      ...
      {({ workspace, user: activeUser }) => {
        const area =
          workspace.type === "admin"
            ? "admin"
            : workspace.type === "creator"
              ? "creator"
              : workspace.type.includes("agency")
                ? "agency"
                : "brand";
        const rows = demoNotifications[activeUser.key];
                  ...
                  <tbody>
                    {rows.map((row) => (
                      <tr className="border-t border-border" key={row.key}>
                        ...
                        <td className="px-4 py-3">{t("unread")}</td>
                        <td className="px-4 py-3">
                          {t(`rows.${row.key}.entity`)}
                        </td>
                        <td className="px-4 py-3">
                          <Link
                            className="font-semibold text-primary"
                            href={withAsParam(row.href, activeUser)}
                          >
